...
public class NPCMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(thisTransform.position.x > xMin && thisTransform.position.x < xMax)
        {
            thisTransform.position += moveDirections[currentMoveDirection] * Time.deltaTime * moveSpeed;
        }
        else if(thisTransform.position.x < xMin)
        {
            thisTransform.position += Vector3.right * Time.deltaTime * moveSpeed;
        }
        else
        {
            thisTransform.position += Vector3.left * Time.deltaTime * moveSpeed;
        }
